...
export default function Desc(props: DescProps): React.ReactElement {
  const { clicked, cityName, cityDetailedName, ppxLoading, ppxError, onClose } =
    props;
  ...
  const displayCity = cityDetailedName || cityName || "Locating city…";
      ...
      {!clicked ? (
        <div className="desc-panel desc-panel--empty">
          <div className="desc-empty" aria-live="polite">
            <div className="desc-empty__icon" role="img" aria-label="Map hint">
              🗺️
            </div>
            <p className="desc-empty__title">Ready to explore?</p>
            <p className="desc-empty__text">
              Tap a location on the map to unlock a tailored briefing.
            </p>
          </div>
        </div>
      ) : (
          ...
          <div className="desc-scroll">
            <header className="desc-header">
              ...
              <div className="desc-header__meta">
                {formattedLat && (
                  <span className="desc-header__meta-item">
                    Lat {formattedLat}
                  </span>
                )}
                {formattedLon && (
            ...
            {!mediaLoading && mediaItem && (
              <figure className="desc-media">
                <img
                  src={mediaItem.url}
                  alt={mediaItem.caption || `Scene from ${displayCity}`}
                />
                {mediaItem.caption && (
                  <figcaption>
            ...
            {!mediaLoading && mediaError && (
              <div className="desc-media desc-media--error">{mediaError}</div>
            )}
            ...
            <div className="desc-body">
              <section className="desc-overview">
                <div className="desc-overview__header">
                  ...
                  {isCityKnown && (
                    <p>
                      A rapid briefing covering history, geography, and standout
                      details.
                    </p>
                  )}
                </div>
                <div className="desc-overview__body">
                  ...
                  {!descLoading && !descError && !descText && isCityKnown && (
                    <p className="desc-body__placeholder">
                      No overview is available right now. Try selecting a nearby
                      city.
                    </p>
                  )}
