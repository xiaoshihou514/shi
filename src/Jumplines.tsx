...
export default function Jumplines(props: Props): React.ReactElement | null {
    ...
    const pathWithTimestamps = useCallback((src: [number, number], dst: [number, number]) => {
        const gc = turf.greatCircle(turf.point(src), turf.point(dst), {npoints: 64});
    ...
    const parseCityList = useCallback((text: string): JumpCity[] => {
        const coerce = (arr: unknown[]): JumpCity[] => arr.map((x) => {
        ...
        } catch { /* no-op: translation not critical */ }
        ...
        if (first !== -1 && last !== -1 && last > first) {
            try {
                ...
                if (Array.isArray(parsed)) return coerce(parsed);
            } catch { /* no-op: fallback failed */ }
        }
        return [];
    }, []);

    const geocodeCity = useCallback(async (name: string, signal?: AbortSignal): Promise<[number, number] | null> => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const concurrency = 4;
                let idx = 0;
                await Promise.all([...Array(concurrency)].map(async () => {
    ...
    useEffect(() => {
        ...
        layers.push(new ArcLayer<JumpDatum>({
            id: 'jump-arcs',
            data: jumps,
            ...
            greatCircle: true,
            getHeight: 0,
            transitions: { getWidth: 600, getSourceColor: 600, getTargetColor: 600 },
        }));

        // Animated highlight traveling along arcs using TripsLayer
        const tripsData = jumps.map(j => pathWithTimestamps([origin.lon, origin.lat], j.target));
        ...
        layers.push(new ScatterplotLayer<JumpDatum>({
            id: 'jump-targets',
            data: jumps,
            getPosition: (d: JumpDatum) => d.target,
            getFillColor: (d: JumpDatum) => [...colorByCategory(d.category), 220] as [number, number, number, number],
            radiusMinPixels: 2,
            radiusMaxPixels: 40,
        ...
        // Labels
        layers.push(new TextLayer<JumpDatum>({
            id: 'jump-labels',
            data: jumps,
            getPosition: (d: JumpDatum) => d.target,
