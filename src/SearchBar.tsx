...
export default function SearchBar(props: SearchBarProps): React.ReactElement {
  ...
  const geocodeCity = useCallback(
    async (name: string, signal?: AbortSignal): Promise<GeocodeResult> => {
  ...
  const pickRandomCityViaPPX = useCallback(
  ...
  const locateByName = useCallback(
    async (name: string) => {
      setBusy(true);
      setError(null);
      try {
  ...
  const wrapClass = useMemo(
    () =>
      ["searchbar-wrap", visible ? "is-visible" : "", className ?? ""]
        .filter(Boolean)
        .join(" "),
    [visible, className],
  );
