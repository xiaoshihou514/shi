...
function useResizeObserver<T extends HTMLElement>(cb: (rect: DOMRectReadOnly) => void) {
...
const ErrorView = ({ message, onRetry }: { message: string; onRetry: () => void }) => (
  <div style={{position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: '#c0392b'}}>
    <div>
      <div style={{ marginBottom: 8 }}>Failed to generate keywords: {message}</div>
...
const Wordcloud = forwardRef<WordcloudHandle, WordcloudProps>((props, ref) => {
  ...
  const containerRef = useResizeObserver<HTMLDivElement>((r) => {
    setRect({ width: Math.max(0, Math.floor(r.width)), height: Math.max(0, Math.floor(r.height)) });
  });
    ...
    addKeywords: (words: Array<{ word: string; weight?: number }>) => {
      setKeywords(prev => {
        ...
        for (const w of words) {
          ...
          exists.add(key);
          additions.push({ word: w.word, weight: clamp(w.weight ?? 1, 1, 100) });
  ...
  const runLayout = useCallback((words: WordItem[], width: number, height: number) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError((e as { message?: string })?.message ?? 'Failed to fetch keywords');
      } finally {
  ...
  const onRetry = useCallback(() => {
    ...
    (async () => {
      try {
        ...
        setError((e as { message?: string })?.message ?? 'Failed to fetch keywords');
      } finally {
        setLoading(false);
      }
    })();
          ...
          {placed.map(word => (
            ...
            >
              <motion.text
                ...
                fontSize={word.size}
                fill={word.fill}
                style={{ cursor: 'default', userSelect: 'none', fontWeight: 700 }}
  ...
  return (
    <div ref={containerRef} className={className} style={{ ...defaultContainerStyle, ...style }}>
      {svg}
