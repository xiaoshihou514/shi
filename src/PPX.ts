...
export async function proSearchText(
  params: StreamProSearchParams & { signal?: AbortSignal },
): Promise<{
  text: string;
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && parsed !== null) {
      ...
      if (typeof cityVal === "string" && typeof detailedNameVal === "string") {
        ...
        if (cityTrimmed.length > 0 && detailedNameTrimmed.length > 0)
          return {
            city: cityTrimmed,
...
export async function normalSearchText(
  params: StreamProSearchParams & { signal?: AbortSignal },
): Promise<{
  text: string;
  usage: ProSearchStreamItem["usage"];
...
function coerceLifeHops(rawArr: unknown[]): LifeHop[] {
  ...
  for (const it of rawArr) {
    ...
    const hop: LifeHop = {
      ...
      sources: Array.isArray(sourcesRaw)
        ? ((sourcesRaw as unknown[])
            .map((s) => {
              const so = (s ?? {}) as Record<string, unknown>;
              const title =
                typeof so["title"] === "string" ? so["title"].trim() : "";
  ...
  for (const h of hops) {
    ...
    if (prev && prev.city.trim().toLowerCase() === h.city.trim().toLowerCase())
      continue;
    dedup.push(h);
  }
  return dedup;
...
export async function fetchPersonLifePath(
  personName: string,
  signal?: AbortSignal,
): Promise<LifeHop[]> {
  ...
  const prompt = [
    ...
    `- Include major relocations or multi-year stays; omit trivial short trips.`,
    `- Birth city (start point) and death city (end point) should be the must-have items in the array.`,
