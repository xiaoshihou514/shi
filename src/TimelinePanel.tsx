...
export default function TimelinePanel(
  ...
  const [activeView, setActiveView] = React.useState<"timeline" | "wordcloud">(
    "timeline",
  );
      ...
      <div className="timeline-panel" aria-live="polite">
        <header className="timeline-panel__header">
          ...
          <h2 className="timeline-panel__title">
            {hasCity
              ? `Key moments in ${displayCity}`
              : "Select a city to begin"}
          </h2>
          <div className="timeline-panel__status">
            {ppxLoading && (
              <span className="timeline-pill timeline-pill--loading">
                Compiling events…
              </span>
            )}
            {!ppxLoading && ppxError && (
        ...
        <div className="timeline-panel__body">
          <div
            className="timeline-panel__view-toggle"
            role="tablist"
          ...
          <div className="timeline-panel__content">
            {activeView === "timeline" ? (
              <div className="timeline-panel__timeline">
                <Timeline
                  ref={timelineRef}
                  className="timeline-compact timeline-panel__timeline-inner"
                />
                {!ppxLoading && !ppxError && !hasCity && (
                  <div className="timeline-panel__notice">
                    Identifying the nearest locale; events will populate
                    shortly.
                  </div>
                )}
              </div>
              ...
              <div className="timeline-panel__wordcloud">
                {hasCity ? (
                  ...
                  <div className="timeline-panel__placeholder">
                    <div
                      className="timeline-panel__placeholder-icon"
                      role="img"
                      aria-label="Word cloud hint"
                    >
                      ☁️
                    </div>
                    <p className="timeline-panel__placeholder-title">
                      Word cloud unavailable
                    </p>
                    <p className="timeline-panel__placeholder-text">
                      Choose a location to surface distinctive themes and
                      highlights.
                    </p>
                  </div>
                )}
