...
export default function DataVizMap(): React.ReactElement {
  ...
  const [featureCollectionByCode, setFeatureCollectionByCode] = useState<
    Record<string, FeatureCollection | undefined>
  >({});
  const [boundaryDataByCode, setBoundaryDataByCode] = useState<
    Record<string, BoundaryPolygon[]>
  >({});
  ...
  useEffect(() => {
    ...
    const toFetch = selectedList.filter(
      (code) => !featureCollectionByCode[code] && !loadingMap[code],
    );
    ...
    toFetch.forEach((code) => {
      ...
      (async () => {
        try {
          ...
          if (!gjResp.ok)
            throw new Error(`GeoJSON fetch failed (${gjResp.status})`);
          const data = (await gjResp.json()) as FeatureCollection<
            Geometry,
            Record<string, unknown>
          >;

          const processed = extractPolygonsFromFeatureCollection(
            code,
          ...
          if (!(err instanceof DOMException && err.name === "AbortError")) {
            const message =
              err instanceof Error ? err.message : "Failed to load boundary";
  ...
  const layers = useMemo<Layer[]>(() => {
    ...
    const baseFill = hexToRgba(baseColorHex, 110) as [
      number,
      number,
      number,
      number,
    ];
    const baseLine = hexToRgba(baseColorHex, 200) as [
      number,
      number,
      number,
      number,
    ...
    for (const code of selectedList) {
      ...
      if (colorHex) {
        const fillColor = hexToRgba(colorHex, 140) as [
          number,
          number,
          number,
          number,
        ];
        const lineColor = hexToRgba(colorHex, 230) as [
          number,
          number,
          number,
          number,
    ...
    if (basePolygons.length) {
      layersOut.push(
        new PolygonLayer<BoundaryPolygon>({
    ...
    if (coloredPolygons.length) {
      layersOut.push(
        new PolygonLayer<ColoredPolygon>({
  ...
  const handleInsightSubmit = useCallback(
    async (event?: React.FormEvent<HTMLFormElement>) => {
      ...
      const countryLines = selectedList
        .map((code) => {
        ...
        if (!ctrl.signal.aborted) {
          ...
          if (parsed.values.length === 0) {
            ...
            if (parsed.intervals && parsed.intervals.length) {
              setIntervalBuckets(
                applyParsedIntervals(parsed.values, parsed.intervals),
              );
            } else {
      ...
      <aside className="altmap-hint">
        ...
        <div className="country-selector">
            ...
            <div className="country-selector__status">
              {selectedList.map((code) => {
                ...
                return (
                  ...
                  >
                    <span className="country-selector__status-code">
                      {code}
                    </span>
            ...
            <div className="insight-legend">
              {intervalBuckets.map((bucket) => (
                <div key={bucket.label} className="insight-legend__item">
                  <span
                    className="insight-legend__swatch"
          ...
          {numericResults.length > 0 && (
            <div className="insight-table">
              {numericResults.map(({ code, value }) => {
                const bucket = intervalBuckets.find((b) =>
                  b.members.some((m) => m.code === code),
                );
                ...
                return (
                  <div key={`numeric-${code}`} className="insight-table__row">
                    ...
                    <span className="insight-table__value">
                      {value.toFixed(2)}
                    </span>
                    <span
                      className="insight-table__tag"
          ...
          <div className="insight-output">
            {insightLoading && (
              <div className="insight-output__status">Fetching data…</div>
            )}
            {insightError && (
...
function extractPolygonsFromFeatureCollection(
  code: string,
  name: string,
...
function parseNumericResponse(
  ...
  try {
    ...
    if (values.length === 0) {
      return {
        values: [],
        error: "Response lacked numeric values for selected codes.",
      };
    }
    ...
    const intervalsNode =
      ...
      typeof parsed === "object" &&
      parsed !== null &&
      Array.isArray((parsed as Record<string, unknown>).intervals)
        ? ((parsed as Record<string, unknown>).intervals as Array<
            Record<string, unknown>
          >)
        : [];
    ...
    const message =
      err instanceof Error ? err.message : "Unable to parse response.";
...
function buildBuckets(values: NumericResult[]): IntervalBucket[] {
  ...
  const bucketCount = Math.min(
    INTERVAL_COLORS.length,
    Math.max(3, Math.min(6, values.length)),
  );
  const span = max - min;
  ...
  for (const entry of values) {
    const bucket =
      buckets.find((b, idx) => {
        ...
        return (
          entry.value >= b.range[0] &&
          (upperInclusive
            ? entry.value <= b.range[1]
            : entry.value < b.range[1])
        );
...
function applyParsedIntervals(
  values: NumericResult[],
  parsedIntervals: ParsedInterval[],
): IntervalBucket[] {
