...
export default defineConfig({
  ...
  plugins: [
    react(),
    {
      ...
      // Ensure the same API routes exist when running `vite preview`
      // so production previews don't 404 on /api/ppx/* endpoints.
      configurePreviewServer(server) {
        ...
        server.middlewares.use("/api/ppx/chat", (req, res, next) => {
          handleChatRequest(req as unknown as IncomingMessage, res as unknown as ServerResponse, next as () => void);
        });
        ...
        server.middlewares.use(
          ...
          async (req, res, next) => {
            ...
            const body = await readJsonBody<MediaBody>(req as unknown as IncomingMessage);
